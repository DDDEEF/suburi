...
void encodeData(FILE *fp, byte *datas, int datasize, int runlengthFlag);
...
int main(int ac, char *av[]){
  ...
  fp_o = fopen(av[2], "wb");
  if(fp_o == NULL){
    fprintf(stderr, "Output File cannot opne\n");
    exit(8);
  }
  encodeData(fp_o, buffer, datasize, 0xff);
  fclose(fp_o);
}
...
/*
  ...
  runlengrhFlagは、ランレングス符号として出力することを示すフラグの値
*/
void encodeData(FILE *fp, byte *datas, int datasize, int runlengrhFlag){
  int i;
  int run, code;
  int processingPoint;
  ...
  while(processingPoint < datasize){
    ...
    if(run <= 2){
      if(code != runlengrhFlag){
        for(i = 0; i < run; i++){
          fputc(code, fp);
        }
      }else{
        fputc(runlengrhFlag, fp);
        fputc(run, fp);
        fputc(code, fp);
      }
    }else{
