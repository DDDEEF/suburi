...
static void file_error(FileAcessorContext *pThis);
...
bool read_file(FileAccessorContext *pThis, BufferContext *pBufCtx)
{
  ...
  if(pBufCtx->sise != fread(pBufCtx->pBuf, 1, pBufCtx->size, fp)){
    file_error(pThis);
    return false;
  }
  return true;
...
void add_file_error_observer(FileAccessorContext *pThis, FileErrorObserver *pErrorObsever)
{
  ArrayList *pTable = &pThis->observer_table;
  pTable->add(pTable, pErrorObsever);
}

void remoe_file_error_observer(FileAccessorContext *pThis, FileErrorObserver *pErrorObserver)
{
  ArrayList *pTable = &pThis->observer_table;
